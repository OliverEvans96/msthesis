...
%DIF ADD main.tex                 Mon Nov 19 14:08:30 2018
...
%DIF 159c159
%DIF < \conferraldate{May}{2018}
%DIF -------
\conferraldate{December}{2018} %DIF > 
%DIF -------
...
Growing concern for the negative environmental impacts of \DIFdelbegin \DIFdel{petroleum-based }\DIFdelend \DIFaddbegin \DIFadd{petroleum--based }\DIFaddend fuel has generated a market for biofuel, especially \DIFdelbegin \DIFdel{corn-based }\DIFdelend \DIFaddbegin \DIFadd{corn--based }\DIFaddend ethanol;
however, \DIFdelbegin \DIFdel{corn-based }\DIFdelend \DIFaddbegin \DIFadd{corn--based }\DIFaddend ethanol has been heavily criticized for diverting land usage away from food production, for increasing use of fertilizers and pesticides that impair water quality, and for the high carbon footprint involved in its development \cite{jones_corn-based_2015}.
...
Specifically, the sugar kelp \textit{Saccharina latissima} has been demonstrated to be a viable source of food, both for direct human consumption and biofuel production, especially in conjunction with other aquatic species in \textit{integrated \DIFdelbegin \DIFdel{multi-trophic }\DIFdelend \DIFaddbegin \DIFadd{multi--trophic }\DIFaddend aquaculture} (IMTA) \cite{brzeski_integrated_1996,chopin_integrating_2001,hadley_modeling_2015,handa_seasonal_2013}.
...
Large scale macroalgae cultivation has long existed in Eastern Asia due to the popularity of seaweed in Asian cuisine, and low labor costs that facilitate its manual seeding and harvest.
  More recently, less \DIFdelbegin \DIFdel{labor-intense }\DIFdelend \DIFaddbegin \DIFadd{labor--intense }\DIFaddend and more industrialized kelp aquaculture has been developing in Scandinavia and in the Northeastern United States and Canada.
...
However, without accounting for \DIFdelbegin \DIFdel{self-shading}\DIFdelend \DIFaddbegin \DIFadd{self--shading}\DIFaddend , the kelp is not adequately penalized for growing too densely,
which is expected to cause overpredictions in the total biomass production.
The purpose of this thesis is to develop a first principles light model which adequately predicts the effects of \DIFdelbegin \DIFdel{self-shading }\DIFdelend \DIFaddbegin \DIFadd{self--shading }\DIFaddend on seaweed.
...
The author takes into account respiration, frond decay, and \DIFdelbegin \DIFdel{sub-canopy }\DIFdelend \DIFaddbegin \DIFadd{sub--canopy }\DIFaddend light attenuation due to \DIFdelbegin \DIFdel{self-shading}\DIFdelend \DIFaddbegin \DIFadd{self--shading}\DIFaddend .
Light attenuation is represented with a simple exponential model, and \DIFdelbegin \DIFdel{self-shading }\DIFdelend \DIFaddbegin \DIFadd{self--shading }\DIFaddend appears as an added term in the decay coefficient.
...
The model developed by Broch et. al. at SINTEF \cite{broch_modelling_2012, broch_modelling_2013, handa_seasonal_2013} uses a \DIFdelbegin \DIFdel{super-individual }\DIFdelend \DIFaddbegin \DIFadd{super--individual }\DIFaddend approach, whereby a small number of individual kelp fronds are explicitly simulated at several discrete depth layers.
Each \DIFdelbegin \DIFdel{super-individual }\DIFdelend \DIFaddbegin \DIFadd{super--individual }\DIFaddend is assumed to represent a certain number of actual individuals in the population.
The number of individuals represented by each \DIFdelbegin \DIFdel{super-individual }\DIFdelend \DIFaddbegin \DIFadd{super--individual }\DIFaddend may change over the course of the simulation due to population loss.
The \DIFdelbegin \DIFdel{super-individual }\DIFdelend \DIFaddbegin \DIFadd{super--individual }\DIFaddend approach has the advantage of capturing some of the dynamics at the individual scale, while compromising full detail for the sake of reduced computational cost.
...
The spatial kelp distribution from Chapter \Rom{\ref{chap:kelp}} is used to determine optical properties of the combined \DIFdelbegin \DIFdel{water-kelp }\DIFdelend \DIFaddbegin \DIFadd{water--kelp }\DIFaddend medium,
and the radiative transfer equation, an \DIFdelbegin \DIFdel{integro-partial }\DIFdelend \DIFaddbegin \DIFadd{integro--partial }\DIFaddend differential equation which describes the the light field as a function of position and angle, is discussed.
...
The \DIFdelbegin \DIFdel{Chapter }\DIFdelend \DIFaddbegin \DIFadd{chapter }\DIFaddend concludes with a comparison to \DIFdelbegin \DIFdel{other }\DIFdelend two simpler light models, and specific qualitative differences are noted.
As expected, the presence of \DIFdelbegin \DIFdel{self-shading }\DIFdelend \DIFaddbegin \DIFadd{self--shading }\DIFaddend in this model results in the prediction of lower light levels regions of high kelp density.
...
\begin{figure}[H]
  ...
  \caption{\textit{Saccharina latissima} \DIFdelbeginFL \DIFdelFL{innoculated }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{inoculated }\DIFaddendFL onto a thread wrapped around a rope on which it is to be grown.}
...
This simplification allows for the \DIFdelbegin \DIFdel{three-dimensionally }\DIFdelend \DIFaddbegin \DIFadd{three--dimensionally }\DIFaddend distributed population of kelp fronds
to be considered a collection of independent populations in \DIFdelbegin \DIFdel{two-dimensional }\DIFdelend \DIFaddbegin \DIFadd{two--dimensional }\DIFaddend depth layers.
...
\DIFaddbegin \DIFadd{The vectors $\vec{x}=(x,y,z)$ and $\vec{\omega}=(\sin\phi\cos\theta, \sin\phi\sin\theta, \cos\phi)$ are also used throughout.
Also, the notation $\hat{x}$, $\hat{y}$, $\hat{z}$ is used for the Cartesian unit vectors.
}

\DIFaddend In the following sections, it is necessary to convert between Cartesian and spherical coordinates, which we do using the relations
\begin{equation}
  \left\{
	\begin{split}
...
\begin{figure}[H]
	...
	\caption{\DIFdelbeginFL \DIFdelFL{Downward-facing right-handed }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Downward--facing right--handed }\DIFaddendFL coordinate system with radial distance $r$ from the origin, distance $s$ from the $z$ axis, zenith angle $\phi$ and azimuthal angle $\theta$.}
	\label{fig:3dcoords}
\end{figure}

\section{Population Distributions}
In order to construct a spatial distribution of kelp fronds, a simple \DIFdelbegin \DIFdel{kite-shaped }\DIFdelend \DIFaddbegin \DIFadd{kite--shaped }\DIFaddend geometry is introduced,
...
\begin{figure}[h]
  ...
  \qquad
	...
	\DIFdelbeginFL %DIFDELCMD < \captionof{figure}{Simplified kite-shaped frond.}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \captionof{figure}{Simplified kite--shaped frond. Reproduced with permission from \cite{broch_modelling_2013}.}
	\DIFaddendFL \label{fig:frond}
\end{figure}

The frond is assumed to be \DIFdelbegin \DIFdel{kite-shaped }\DIFdelend \DIFaddbegin \DIFadd{kite--shaped }\DIFaddend with length $l$ from base to tip, and width $w$ from left to right.
...
Notice that unlike the normal distribution, the von Mises distribution approaches a \textit{\DIFdelbegin \DIFdel{non-zero}\DIFdelend \DIFaddbegin \DIFadd{non--zero}\DIFaddend } uniform distribution as $\kappa$ approaches 0, so
\begin{equation*}
...
\subsection{Joint \DIFdelbegin \DIFdel{Length-Orientation }\DIFdelend \DIFaddbegin \DIFadd{Length--Orientation }\DIFaddend Distribution}
\label{sec:dist_2d}
...
\begin{figure}[h]
	...
	\DIFdelbeginFL %DIFDELCMD < \captionof{figure}{2D length-angle probability distribution with $\theta_w=7\pi/4$, $v_w=1$, $\mu_l=3$, $\sigma_l=1$.}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \captionof{figure}{2D length--angle probability distribution with $\theta_w=7\pi/4$, $v_w=1$, $\mu_l=3$, $\sigma_l=1$.}
	\DIFaddendFL \label{fig:dist_2d}
\end{figure}
...
This is made possible by the simple \DIFdelbegin \DIFdel{kite-shape }\DIFdelend \DIFaddbegin \DIFadd{kite--shape }\DIFaddend fronds, and would be considerably more difficult with more general frond shapes.
...
\begin{figure}[h]
	...
	\DIFdelbeginFL %DIFDELCMD < \captionof{figure}{Outlines of minimum-length fronds for a variety of angles to occupy the point $(\theta,s)=(3\pi/4,3/2)$.}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \captionof{figure}{Outlines of minimum--length fronds for a variety of angles to occupy the point $(\theta,s)=(3\pi/4,3/2)$.}
...
\section{Discontinuity at the \DIFdelbegin \DIFdel{rope}\DIFdelend \DIFaddbegin \DIFadd{Rope}\DIFaddend }
...
Luckily, there is a straightforward solution, which is to \DIFdelbegin \DIFdel{post-process }\DIFdelend \DIFaddbegin \DIFadd{post--process }\DIFaddend the spatial kelp distribution with a Gaussian blur in the $x$ and $y$ dimensions.
...
\subsection{One dimensional Gaussian \DIFdelbegin \DIFdel{blur}\DIFdelend \DIFaddbegin \DIFadd{Blur}\DIFaddend }
...
%DIFDELCMD < %%%
\DIFdelend A physical interpretation of this Gaussian blurring is that $P_k^b$ is the time--averaged kelp distribution, assuming that the rope is allowed to move horizontally, and $k(x, y; \sigma)$ is the PDF of the rope's location distribution.
...
\subsection{Absorption \DIFdelbegin \DIFdel{coefficient plots}\DIFdelend \DIFaddbegin \DIFadd{Coefficient Plots}\DIFaddend }
%DIF <  TODO: Clean up figures
...
In each figure, the absorption coefficient is plotted on the color axis, while the norm of its gradient is shown with contours, with brighter contour lines \DIFdelbegin \DIFdel{representing }\DIFdelend \DIFaddbegin \DIFadd{indicating }\DIFaddend regions of large derivatives.
...
\DIFaddbegin \DIFadd{Note that all of the kelp distributions are periodic in order to easily represent multiple vertical lines grown in close proximity.
}\DIFaddend 

Figure \DIFdelbegin \DIFdel{\ref{fig:kelp_dist_sharp} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:abs_coefs_combined}(a) }\DIFaddend shows a sharp kelp distribution with an unrealistically high current velocity, with all fronds identically equal in length, and with no blurring applied.
...
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=5in]{kelp_dist_sharp}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Absorption coefficient profile in the limiting case of $v_w>>1$, $\sigma_l=0$ and $\sigma_b=0$ demonstrates the kite--shaped fronds. Note the high gradient near the inner edge of the frond.}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:kelp_dist_sharp}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend 

In Figure \DIFdelbegin \DIFdel{\ref{fig:kelp_dist_smooth}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:abs_coefs_combined}(b)}\DIFaddend , the current velocity has been reduced, and a nonzero standard deviation has been given to the frond distribution.
...
Such a distribution \DIFdelbegin \DIFdel{would }\DIFdelend \DIFaddbegin \DIFadd{may }\DIFaddend still cause numerical difficulties for the numerical algorithm.

\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <   \centering
%DIFDELCMD <   \vspace{-3em}
%DIFDELCMD <   \includegraphics[width=5in]{kelp_dist_smooth}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Absorption coefficient profile for realistic values of $v_w$ and $\sigma_l$, but with $\sigma_b=0$. Note the large gradient at the origin. By continuing to increase the plotting resolution, the gradient would be found to be unbounded.}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:kelp_dist_smooth}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Figure \ref{fig:kelp_dist_smooth_smallblur} }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:abs_coefs_combined}(c) }\DIFaddend is similar to Figure \DIFdelbegin \DIFdel{\ref{fig:kelp_dist_smooth}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:abs_coefs_combined}(b)}\DIFaddend , except it has been post--processed with a \DIFdelbegin \DIFdel{small }\DIFdelend \DIFaddbegin \DIFadd{moderate }\DIFaddend Gaussian blur of radius \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\SI{10}{cm}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\SI{40}{cm}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , not so large that it significantly distorts the shape of the distribution, but sufficient to remove the discontinuity in the origin.
Such a kelp distribution is ideal, as it balances accuracy with ease of numerical approximation.
\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <   \centering
%DIFDELCMD <   \vspace{-3em}
%DIFDELCMD <   \includegraphics[width=5in]{kelp_dist_smooth_smallblur}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{A small blur radius of $\sigma_b=\SI{0.1}{\m}$ eliminates the derivative singularity at the origin.}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:kelp_dist_smooth_smallblur}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend 

In Figure \DIFdelbegin \DIFdel{\ref{fig:kelp_dist_smooth_blur}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:abs_coefs_combined}(d)}\DIFaddend , too large of a blur has been applied; the character of the Gaussian kernel has overwhelmed that of the kelp distribution.
While this may still provide a better spatial description of the kelp than a fully horizontally uniform distribution, and unnecessary amount of information has been sacrificed for marginal gains in smoothness over Figure \DIFdelbegin \DIFdel{\ref{fig:kelp_dist_smooth_smallblur}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:abs_coefs_combined}(c)}\DIFaddend .
Such a large blur is unlikely to reduce the discretization error \DIFdelbegin \DIFdel{, }\DIFdelend since other factors \DIFdelbegin \DIFdel{are contributing to it more significantly}\DIFdelend \DIFaddbegin \DIFadd{discussed later contribute to discretization error as well}\DIFaddend .
\begin{figure}[H]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \vspace{-3em}
%DIFDELCMD <   \includegraphics[width=5in]{kelp_dist_smooth_blur}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=6in]{abs_coefs_combined}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Over-blurring }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$z$--slices of several absorption coefficient distributions from }\DIFaddendFL kelp \DIFdelbeginFL \DIFdelFL{distribution }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{distributions }\DIFaddendFL with \DIFdelbeginFL \DIFdelFL{$\sigma_b=\SI{10}{\m}$ causes it to lose a great deal }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{varying parameters. The norm }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{spatial resolution}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the gradient is depicted with contours}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{This }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(a) $\eta v_w=90$, $\sigma_l=\sigma_b=0$. Unrealistically sharp distribution shows kite--shaped character. (b) $\eta v_w=10$, $\sigma_b=0$, $\sigma_l=\SI{1}{\m}$. More realistic kelp distribution, but still has large derivatives near the origin. (c) $\eta v_w=10$, $\sigma_b=\SI{0.4}{\m}$, $\sigma_l=\SI{1}{\m}$. Moderate Gaussian blur slightly reduces derivatives near the origin. (d) $\eta v_w=10$, $\sigma_b=\SI{2}{\m}$, $\sigma_l=\SI{1}{\m}$. Over--blurred distribution; }\DIFaddendFL should be avoided.}
  \DIFdelbeginFL %DIFDELCMD < \label{fig:kelp_dist_smooth_blur}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:abs_coefs_combined}
...
From the standpoint of human perception, the importance of light is not in the raw energy that it transfers, but the degree to which it facilitates vision \DIFaddbegin \DIFadd{from a physiological perspective}\DIFaddend .
...
In photometric units, radiance becomes luminance, irradiance \DIFdelbegin \DIFdel{becomes }\DIFdelend illuminance, and so on.
...
The radiance $L$, expresses this dependence, and is defined as the radiant flux per steradian per projected surface area perpendicular to the direction of propagation of the beam \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{chandrasekhar_radiative_1960}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
That is,
\begin{equation*}
...
\end{equation*}
This quantity is also called \textit{scalar irradiance} since it does not consider light through particular surface, but rather weights radiance from all directions equally \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{kirk_light_1994}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
Formally, given two directions $\vec{\omega}$ and $\vec{\omega}'$, $\beta(\vec{\omega} \cdot \vec{\omega}')$ is the probability density of light scattering from $\vec{\omega}$ into $\vec{\omega}'$ (or \DIFdelbegin \DIFdel{vice-versa}\DIFdelend \DIFaddbegin \DIFadd{vice--versa}\DIFaddend ).
...
\begin{equation*}
  \int_{4\pi}\beta(\vec{\omega}\cdot\vec{\omega}')\, d\vec{\omega}' = \DIFdelbegin \DIFdel{1.
...
\DIFaddbegin \label{sec:ray_notation}
\DIFaddend Consider a fixed position $\vec{x}$ and direction $\vec{\omega}$ such that
...
\end{align}
Then, we convert back from path length $s$ to the spatial coordinate $\vec{x}$ by evaluating the \DIFdelbegin \DIFdel{one-dimensional }\DIFdelend \DIFaddbegin \DIFadd{one--dimensional }\DIFaddend solution at the end of the ray path.
That is,
\begin{equation*}
...
In this \DIFdelbegin \DIFdel{Chapter}\DIFdelend \DIFaddbegin \DIFadd{chapter}\DIFaddend , the mathematical details involved in the numerical solution of the equations described in Chapters \ref{chap:kelp} and \ref{chap:light} are presented.
Two solution algorithms are used: finite difference and numerical asymptotics.
Both algorithms require a discrete \DIFdelbegin \DIFdel{spatial-angular }\DIFdelend \DIFaddbegin \DIFadd{spatial--angular }\DIFaddend grid, described in Section \ref{sec:grid}.
In the finite difference algorithm, the \DIFdelbegin \DIFdel{continuuum }\DIFdelend \DIFaddbegin \DIFadd{continuum }\DIFaddend differential equation is approximated at every point in the grid.
...
The following is a description of the \DIFdelbegin \DIFdel{spatial-angular }\DIFdelend \DIFaddbegin \DIFadd{spatial--angular }\DIFaddend grid used in the numerical implementation of this model.
...
\DIFaddbegin \DIFadd{Note that $n_\phi$ must be even in order to avoid perfectly horizontal rays, as discussed in Section \ref{sec:ray_notation} and Appendix \ref{chap:grid_details}.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{One-indexing }\DIFdelend \DIFaddbegin \DIFadd{One--indexing }\DIFaddend is employed throughout this document (i.e., array elements are counted starting at 1, not 0).
...
The \DIFdelbegin \DIFdel{three-dimensional }\DIFdelend \DIFaddbegin \DIFadd{three--dimensional }\DIFaddend interval centered at $(x_i, y_j, z_k)$ is denoted $X_{ijk}$, and has volume $\abs{X_{ijk}}=dx\,dy\,dz$.
...
Refer to Appendix \ref{chap:grid_details} for a more rigorous discussion of the discrete \DIFdelbegin \DIFdel{spatial-angular }\DIFdelend \DIFaddbegin \DIFadd{spatial--angular }\DIFaddend grid.

\section{Kelp Distribution}
\DIFaddbegin \DIFadd{In order to determine the appropriate spatial dependence for the absorption coefficient to use in the radiative transfer equation, the spatial kelp distribution, described by }\eqref{eqn:kelp_pk} \DIFadd{and its tributaries, is calculated at every point t in space.
...
\subsection{\DIFdelbegin \DIFdel{Super-Individuals}\DIFdelend \DIFaddbegin \DIFadd{Super--Individuals}\DIFaddend }
\label{sec:si}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Rather than model each kelp frond, subsets of the population, called \DIFdelbegin \DIFdel{super-individuals}\DIFdelend \DIFaddbegin \DIFadd{super--individuals}\DIFaddend , are modeled explicitly, and are considered to represent many identical individuals, as in \citep{scheffer_super-individuals_1994}.
Specifically, at each depth $k$, there are $S_k$ \DIFdelbegin \DIFdel{super-individuals}\DIFdelend \DIFaddbegin \DIFadd{super--individuals}\DIFaddend , indexed by $q$.
\DIFdelbegin \DIFdel{Super-individual }\DIFdelend \DIFaddbegin \DIFadd{Super--individual }\DIFaddend $q$ has a frond area $A_{kq}$ and represents $S_{kq}$ individual fronds.

From \eqref{eqn:length_from_area}, the frond length of the \DIFdelbegin \DIFdel{super-individual }\DIFdelend \DIFaddbegin \DIFadd{super--individual }\DIFaddend is $l_{kq} = \sqrt{2A_{kq}f_r}$.
Given the \DIFdelbegin \DIFdel{super-individual }\DIFdelend \DIFaddbegin \DIFadd{super--individual }\DIFaddend data, we calculate the mean $\mu$ and standard deviation $\sigma$ frond
...
\subsection{Integration of \DIFaddbegin \DIFadd{Population }\DIFaddend Distributions}
...
To compute the integral, we symbolically convert the \DIFdelbegin \DIFdel{two-dimensional }\DIFdelend \DIFaddbegin \DIFadd{two--dimensional }\DIFaddend integral to a \DIFdelbegin \DIFdel{one-dimensional }\DIFdelend \DIFaddbegin \DIFadd{one--dimensional }\DIFaddend integral in terms of the error function
\begin{equation}
...
At present, a \DIFdelbegin \DIFdel{Gauss-Legendre }\DIFdelend \DIFaddbegin \DIFadd{Gauss--Legendre }\DIFaddend quadrature with degree 101 is applied.
...
Since it is assumed that all quantities are constant within a \DIFdelbegin \DIFdel{spatial-angular }\DIFdelend \DIFaddbegin \DIFadd{spatial--angular }\DIFaddend grid cell,
the midpoint rule is employed for both spatial and angular integration.
Presented here is a basic derivation of the formulas for integration in the \DIFdelbegin \DIFdel{spatial-angular }\DIFdelend \DIFaddbegin \DIFadd{spatial--angular }\DIFaddend grid.
...
While the asymptotic solution is valid in the case of low scattering, a more general solution is obtained via finite difference, whereby the derivatives and integrals in the \DIFdelbegin \DIFdel{integro-partial }\DIFdelend \DIFaddbegin \DIFadd{integro--partial }\DIFaddend differential equation are discretized to differences and sums and evaluated at each grid cell \DIFdelbegin \DIFdel{in order }\DIFdelend to construct a linear system of equations whose solution approximates that of the analytical equation.
The price of a general solution, \DIFdelbegin \DIFdel{however, }\DIFdelend is greatly increased computational cost, both in terms of memory and CPU usage.
...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
Now, for upwelling light at the first depth layer (\DIFdelbegin \DIFdel{non-BC}\DIFdelend \DIFaddbegin \DIFadd{non--BC}\DIFaddend ), we apply FD2.
\begin{equation*}
  \begin{aligned}
    \sigma_{ijkp} &= \frac{L_{i+1,jkp}-L_{i-1,jkp}}{2dx}\sin\hat{\phi}_p\cos\hat{\theta}_p \\
    &+ \frac{L_{i,j+1,kp}-L_{i,j-1,kp}}{2dy}\sin\hat{\phi}_p\sin\hat{\theta}_p \\
...
In the coefficient matrix $\mathbb{A}$, each row is \DIFdelbegin \DIFdel{asociated }\DIFdelend \DIFaddbegin \DIFadd{associated }\DIFaddend with the grid cell at which the discretized equation was evaluated.
Each column is the coefficient of the radiance at a particular \DIFdelbegin \DIFdel{spatial-angular }\DIFdelend \DIFaddbegin \DIFadd{spatial--angular }\DIFaddend grid cell.

In principle, the order of the equations, i.e., the order of the rows and columns of the coefficient matrix, is not important
so long as consistency is maintained with the solution vector and \DIFdelbegin \DIFdel{right-hand }\DIFdelend \DIFaddbegin \DIFadd{right--hand }\DIFaddend side.
...
and the \DIFdelbegin \DIFdel{right-hand }\DIFdelend \DIFaddbegin \DIFadd{right--hand }\DIFaddend side associated with that grid cell, if any, is also stored at position $r_{ijkp}$ of the \DIFdelbegin \DIFdel{right-hand }\DIFdelend \DIFaddbegin \DIFadd{right--hand }\DIFaddend side vector $\mathbb{B}$.
...
\DIFaddend For example, a $10 \times 10 \times 10 \times 10 \times 10$ grid involves 7,207,800 matrix elements, while a $20 \times 20 \times 20 \times 20 \times 20$ grid requires 1,065,583,200 numbers to be stored!
...
Many such solvers are available, including GMRES \cite{saad_gmres:_1985}, LGMRES \cite{baker_technique_2005}, IDR \cite{sonneveld_idrs:_2008}, and \DIFdelbegin \DIFdel{BI-CGSTAB }\DIFdelend \DIFaddbegin \DIFadd{BI--CGSTAB }\DIFaddend \cite{van_der_vorst_bi-cgstab:_1992}.
...
In this \DIFdelbegin \DIFdel{Chapter}\DIFdelend \DIFaddbegin \DIFadd{chapter}\DIFaddend , the validity of the numerical approximations to the continuum model is discussed.
Concepts relating to determining the correctness of a whole code \DIFaddbegin \DIFadd{and the accuracy }\DIFaddend of specific results are surveyed.
The Fortran implementations of the finite difference and \DIFdelbegin \DIFdel{asymptotic }\DIFdelend \DIFaddbegin \DIFadd{numerical asymptotics }\DIFaddend solutions of the radiative transfer equation are verified \DIFaddbegin \DIFadd{with reasonable confidence }\DIFaddend to be free of detectable coding mistakes.
...
Any assumptions which do not hold precisely for the real system contribute to differences between \DIFdelbegin \DIFdel{model outputs and the real behavior }\DIFdelend \DIFaddbegin \DIFadd{solutions to the model equations and the behavior of the real system}\DIFaddend .
Such differences can be termed \textit{\DIFdelbegin \DIFdel{conceptual modelling }\DIFdelend \DIFaddbegin \DIFadd{modeling }\DIFaddend errors}, and are a present in the exact solutions to the mathematical equations comprising the model.
...
The error resulting from using a finite number of terms from an infinite series is \DIFdelbegin \DIFdel{refered }\DIFdelend \DIFaddbegin \DIFadd{referred }\DIFaddend to as \textit{truncation error}.
...
The loss of accuracy in computations due to the use of floating point numbers is termed \textit{round--off error} \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{roache_verification_1998}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\label{sec:v2}
%DIF < TODO: Citations
\DIFdelbegin \DIFdel{As Roache explains, there }\DIFdelend \DIFaddbegin \DIFadd{There }\DIFaddend are two aspects to building confidence in numerical codes, specifically those which solve PDEs: verification \DIFaddbegin \DIFadd{and validation.
Verification }\DIFaddend deals with \textit{solving the equations right}, while validation deals with \textit{solving the right equations} \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{roache_verification_1998}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
That is, it is the process of checking that \DIFdelbegin \DIFdel{conceptual modelling }\DIFdelend \DIFaddbegin \DIFadd{modeling }\DIFaddend errors are sufficiently small to model the system as accurately as necessary.
Validation is an ongoing process; as new experimental data become available, the equations can be solved under appropriate conditions in an attempt to replicate the new observations \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{roache_building_2004}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
For realistic conditions, however, a second stage of verification is \DIFdelbegin \DIFdel{emplyed}\DIFdelend \DIFaddbegin \DIFadd{employed}\DIFaddend : \textit{verification of calculations}.
...
Of course, there is a \DIFdelbegin \DIFdel{trade-off}\DIFdelend \DIFaddbegin \DIFadd{trade--off}\DIFaddend : the solutions are not physically realistic.
...
Here, $u$ is \DIFdelbegin \DIFdel{hand-picked }\DIFdelend \DIFaddbegin \DIFadd{hand--picked }\DIFaddend at the outset to be easy to calculate, all parameters and coefficient functions in $D$ are chosen to be nonzero, and the source term $\sigma$ which produces the desired solution is calculated.
...
Whereas \textit{inverting} a differential operator analytically is impossible for many equations and often requires ingenuity when it is, \textit{applying} one is a \DIFdelbegin \DIFdel{plug-and-chug }\DIFdelend \DIFaddbegin \DIFadd{plug--and--chug }\DIFaddend application of algebra and calculus.
...
Also, $u$ must satisfy any constraints imposed by the algorithm such as \DIFdelbegin \DIFdel{hard-coded }\DIFdelend \DIFaddbegin \DIFadd{hard--coded }\DIFaddend boundary conditions or acceptable coefficient ranges.
Finally, the chosen functions should have small derivatives so that convergence can be achieved for reasonable grid sizes.
Since these functions may need to be fairly complicated in order to achieve full generality while meeting the necessary constraints, it is advisable to use a \textit{Computer Algebra System} (CAS) such as the Python package Sympy \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{meurer_sympy:_2017} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend to symbolically compute the source term.
%DIF <  TODO: Cite Sympy
...
Numerical calculations are computed on a sequence of discrete grids from coarse to fine, and errors are calculated by evaluating the exact \DIFdelbegin \DIFdel{soution }\DIFdelend \DIFaddbegin \DIFadd{solution }\DIFaddend pointwise.
...
If the verification procedure shows that the implementation does not demonstrate the theoretical order of accuracy of the algorithm, this is most likely \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{roache_verification_1998-1} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend an indication that either a coding mistake is present or the asymptotic range has not been achieved.
%DIF <  TODO: Cite roache after "most likely"
...
\begin{enumerate}
  ...
  \item \DIFdelbegin \DIFdel{Position-independent }\DIFdelend \DIFaddbegin \DIFadd{Position--independent }\DIFaddend surface downwelling boundary condition
...
%DIF <  TODO: Discuss OAMs (order of accuracy mistakes)
%DIF < The only source of error in the finite difference method is discretization error due to the truncation of terms in the Taylor Series during the construction of the finite difference formulas in Section \ref{sec:discretization}.
...
This reduces the \DIFdelbegin \DIFdel{dimenionality }\DIFdelend \DIFaddbegin \DIFadd{dimensionality }\DIFaddend of the resolution space to two, but it would be \DIFdelbegin \DIFdel{preferrable }\DIFdelend \DIFaddbegin \DIFadd{preferable }\DIFaddend to deal only with a single variable.
Therefore, the finite difference verification is performed by holding $n_a=8$, and varying $n_s$ between 4 and 64.
As shown in Figure \ref{fig:fd_mms}, second order convergence is observed, demonstrating that the code \DIFdelbegin \DIFdel{works without order-of-accuracy mistakes .
%DIF <  TODO: Discuss multidimensional error further
%DIF <  TODO: Discuss log-log plot.
}\DIFdelend \DIFaddbegin \DIFadd{is free of coding mistakes affecting the convergence properties of the algorithm.
}\DIFaddend 

\begin{figure}[h]
  \centering
  \includegraphics[width=5in]{fd_mms}
  \caption{Code verification for the finite difference solution. Each point represents the same simulation run with a different spatial grid sizes, with the angular grid held constant at $n_a=8$. A slope of $m=2$ on a \DIFdelbeginFL \DIFdelFL{log-log }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{log--log }\DIFaddendFL scale demonstrates second order convergence, as expected, demonstrating the correctness of the code.}
  \label{fig:fd_mms}
\end{figure}
...
As with discretization error, this can only be observed within some asymptotic range of small $b$ values \DIFaddbegin \DIFadd{since }\DIFaddend higher order error terms dominate \DIFaddbegin \DIFadd{the truncation error }\DIFaddend for large values of $b$.
...
It is unclear whether this is due to a coding mistake, the effect of discretization error, or if there is another cause for the \DIFdelbegin \DIFdel{sub-optimal }\DIFdelend \DIFaddbegin \DIFadd{sub--optimal }\DIFaddend convergence.
\begin{figure}[h]
  \centering
...
\DIFaddbegin \DIFadd{This type of error reporting is crucial for accurately communicating the trustworthiness of simulation results, and has long been a common requirement for journals in some computational fields such as computational fluid dynamics (CFD) \mbox{%DIFAUXCMD
\cite{roache_editorial_1986}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 
...
\DIFaddbegin \label{sec:generalized_re}
\DIFaddend Of course, the above equations are only approximations.
...
Additionally, this take on Richardson extrapolation is trivially applied to multiple \DIFdelbegin \DIFdel{dimenisions}\DIFdelend \DIFaddbegin \DIFadd{dimensions}\DIFaddend .
For a grid which has several resolution parameters $h_1, h_2, \ldots, h_n$ (e.g. multiple spatial dimensions, or spatial and angular grid resolutions), if the algorithm is \DIFdelbegin \DIFdel{second-order }\DIFdelend \DIFaddbegin \DIFadd{second--order }\DIFaddend in each resolution parameter, then
\begin{equation*}
  \phi \approx \phi_e + g_{21} h_1^2 + g_{22} h_2^2 + \ldots + g_{2n} h_n^2.
\end{equation*}
Hence, fitting a plane or \DIFdelbegin \DIFdel{hyper-plane }\DIFdelend \DIFaddbegin \DIFadd{hyper--plane }\DIFaddend through several points in $(h_1^2, h_2^2, \ldots, h_n^2, \phi)$ space similarly produces $\phi_e$ as the $\phi$--intercept.

\chapter{PRACTICAL APPLICATION}
\label{chap:application}

This \DIFdelbegin \DIFdel{Chapter }\DIFdelend \DIFaddbegin \DIFadd{chapter }\DIFaddend deals with the practical considerations when applying the previously discussed numerical algorithms to simulate the light field for realistic scenarios of kelp growing in ocean waters.
This \DIFdelbegin \DIFdel{involves combining the }\DIFdelend \DIFaddbegin \DIFadd{is chapter is the culmination of this thesis: the }\DIFaddend spatial kelp distribution model of Chapter \Rom{\ref{chap:kelp}} \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the radiative transfer model of Chapter \Rom{\ref{chap:light}} \DIFdelbegin \DIFdel{according to the numerical details }\DIFdelend \DIFaddbegin \DIFadd{are combined using the numerical algorithms }\DIFaddend described in Chapter \Rom{\ref{chap:numerical}} and verified in Chapter \Rom{\ref{chap:model_analysis}} \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{and applied to a realistic scenario
}\DIFaddend 
...
Guidelines are presented in this \DIFdelbegin \DIFdel{Chapter }\DIFdelend \DIFaddbegin \DIFadd{chapter }\DIFaddend to aid the user in making these decisions, and the performance of the model is discussed.
Finally, comparison is made to simpler light models, and specific differences are noted.
With all of this information, a potential user may decide if and how to use the \DIFaddbegin \DIFadd{three--dimensional light }\DIFaddend model presented in this thesis.

\section{Physical Parameters}
\label{sec:parameters}
In this \DIFdelbegin \DIFdel{Section}\DIFdelend \DIFaddbegin \DIFadd{section}\DIFaddend , physical model parameters are discussed.
...
In that case, \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{those models }\DIFaddend will provide some of the necessary parameters such as the size of the kelp fronds, optical properties of the aquatic medium, and current speed as functions of depth.
If the light model is run without kelp growth and ocean models, as is the case for the results in this \DIFdelbegin \DIFdel{Chapter}\DIFdelend \DIFaddbegin \DIFadd{chapter}\DIFaddend , then these parameters must be hand--picked to represent a realistic \DIFdelbegin \DIFdel{scenario}\DIFdelend \DIFaddbegin \DIFadd{situation of interest}\DIFaddend .
...
and was converted to \SI{}{\W\per\m\squared} according to \DIFaddbegin \DIFadd{the approximate relation }\DIFaddend \eqref{eqn:watts_photons}.
\DIFdelbegin \DIFdel{No data in the literature exist for the frond thickness, so a best estimate is provided.
}\DIFdelend 
...
\begin{table}[h]
  ...
  \begin{tabular}{lrrr}
    ...
    Frond thickness & $f_t$ & \SI{0.4}{\mm} & \DIFaddbeginFL \DIFaddFL{\mbox{%DIFAUXCMD
\cite{foldal_morphological_2018} }\hspace{0pt}%DIFAUXCMD
}\\
    \DIFaddFL{Frond aspect ratio }& \DIFaddFL{$f_r$ }& \DIFaddFL{5.0 }& \DIFaddFL{\mbox{%DIFAUXCMD
\cite{foldal_morphological_2018} }\hspace{0pt}%DIFAUXCMD
}\\
...
\DIFaddbegin \DIFadd{These values give a sense for the range of absorption and scattering coefficients that the model should be tested against.
Aside from absorption and scattering coefficients, detailed tabulations of volume scattering functions are also given in \mbox{%DIFAUXCMD
\citep{petzold_volume_1972}}\hspace{0pt}%DIFAUXCMD
.
While a simpler linear model is used for the simulations in this thesis, it is recommended that future simulations interpolate the tabulated VSF from \mbox{%DIFAUXCMD
...
If the fluorescent dye is applied, then a simple \DIFdelbegin \DIFdel{peak-finding }\DIFdelend \DIFaddbegin \DIFadd{peak--finding }\DIFaddend image processing algorithm can be applied to locate the frond tips.
...
The kelp growth model is expected to provide \DIFdelbegin \DIFdel{super-individual }\DIFdelend \DIFaddbegin \DIFadd{super--individual }\DIFaddend data describing the population in each depth layer.
...
While the vertical resolution of the encompassing simulation is probably finer than the horizontal resolution, it may also not be sufficiently fine to use for \DIFdelbegin \DIFdel{the light model in order to obtain a sufficiently accurate solution}\DIFdelend \DIFaddbegin \DIFadd{light field calculations}\DIFaddend .
...
\DIFaddbegin \DIFadd{A value of $D_s=\SI{1}{\per\radian}$ is used for the simulations in this chapter.
}\DIFaddend 


\subsection{Standalone Context}
\label{sec:standalone_context}
The simulation results shown in the later \DIFdelbegin \DIFdel{Sections of this Chapter }\DIFdelend \DIFaddbegin \DIFadd{sections of this chapter }\DIFaddend probe the light model in various ways independent of any encompassing kelp growth or ocean models, and therefore the physical parameters that they would supply must be hand--picked to represent a realistic scenario to which the light model may be applied.
...
%DIF <  TODO: Add data from Ole Jacob's student's thesis
\section{\DIFdelbegin \DIFdel{Algorithm Parameters}\DIFdelend \DIFaddbegin \DIFadd{Computational Expense}\DIFaddend }
...
%DIF <  TODO: Cite OpenMP
Further, some sections of the code are necessarily serial, which reduces the effective parallelism to some degree.
\DIFdelbegin \DIFdel{For }\DIFdelend \DIFaddbegin \DIFadd{When }\DIFaddend using 8 threads, an average CPU usage of about \DIFdelbegin \DIFdel{5 }\DIFdelend \DIFaddbegin \DIFadd{7 }\DIFaddend cores is observed over the course of the \DIFaddbegin \DIFadd{entire }\DIFaddend computation, while for 32 threads, it is around 22. 
%DIF <  TODO: Check actual values.
\DIFaddbegin \DIFadd{The parallel efficiency decreases as thread count increases because the serial sections bias the average parallelism downward.
}\DIFaddend This is not intended to discourage parallelism, only to represent it accurately; in reality, the speedup is quite noticeable when the thread count is increased.
...
It runs until an error criterion is satisfied\DIFdelbegin \DIFdel{for the matrix equation}\DIFdelend , and therefore the exact number of computations is not known ahead of time.
...
\DIFdelendFL \DIFaddbeginFL \begin{figure}[H]
  ...
  \caption{Computation time required for numerical asymptotics and finite difference algorithms over a range of spatial grid sizes using 32 CPUs. Only five grid sizes are shown, with the finite difference and numerical asymptotic algorithm for $n=0,\ldots,3$ terms shown for each grid. The horizontal offset within each grid size is only for visual clarity. \DIFdelbeginFL \DIFdelFL{TODO: Replace violin plots with box--and--whisker plots}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Note the large range in compute times --- small simulations take fractions of a second while large grids take upwards of a half--hour}\DIFaddendFL .}
...
\begin{figure}[H]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in]{memory_solve}
%DIFDELCMD <   %%%
...
%DIF < \section{Grid Size and Discretization Error}
\DIFaddbegin \section{\DIFadd{Grid Size and Discretization Error}}
\label{sec:grid_size}
\DIFadd{The size of the spatial--angular grid is an important choice that must be made in order to balance numerical accuracy with computational cost.
...
\subsection{\DIFadd{Error Estimation}}
\DIFadd{A set of simulations is run for a range of spatial and angular grid sizes with optical properties from Petzold's coastal California waters (HAOCE11), listed in Table \ref{tab:petzold}.
...
The average irradiances for each simulation are plotted in Figures \ref{fig:gs_avg_irrad_contourf} and \ref{fig:gs_avg_irrad_lines} as functions of the squared grid spacings $ds^2$ and $da^2$, given by Equations }\eqref{eqn:ds} \DIFadd{and }\eqref{eqn:da} \DIFadd{with $\xmax-\xmin = \ymax-\ymin = \SI{10}{\m}$.
...
\begin{figure}[H]
  ...
  \caption{\DIFaddFL{Average irradiance plotted against squared resolution for a variety of grid sizes for finite difference and numerical asymptotics with $n=0$. The linear contours demonstrate that both methods are second order in both resolution parameters.}}
  \label{fig:gs_avg_irrad_contourf}
\end{figure}

\DIFadd{Notice that in Figure \ref{fig:gs_avg_irrad_lines}, the average irradiance is linear in squared grid spacing for each slice in the resolution space, which 
...
\DIFadd{Specifically, the discretization error for each algorithm is found to be
}\begin{align}
  \DIFadd{\varepsilon_{\mbox{FD}}(ds, da) }&\DIFadd{= c_{11} ds^2 + c_{12} da^2, }\\
  \DIFadd{\varepsilon_{\mbox{asym}}(ds, da) }&\DIFadd{= c_{21} ds^2 + c_{22} da^2,
}\end{align}
\DIFadd{where
}\begin{align}
  \DIFadd{c_{11} }&\DIFadd{= 0.180, }& \DIFadd{C_{12} }&\DIFadd{= 0.183, }\\
  \DIFadd{c_{21} }&\DIFadd{= -0.851, }& \DIFadd{C_{22} }&\DIFadd{= 0.247.
}\end{align}

\DIFadd{By evaluating this plane at $ds=0$, $da=0$, the extrapolated continuum value of average irradiance is found.
...
\DIFadd{For example, take the asymptotics algorithm with the grid $n_s=60$, $n_a=20$.
...
The angular discretization error for this particular grid looks to be about \mbox{%DIFAUXCMD
\SI{2e-1}{\W\per\m\squared}}\hspace{0pt}%DIFAUXCMD
, while the spatial discretization error is about \mbox{%DIFAUXCMD
\SI{3e-2}{\W\per\m\squared}}\hspace{0pt}%DIFAUXCMD
.
...
\DIFadd{From the other perspective, consider an overall target error of $\bar{\varepsilon}=\SI{2e-2}{\W\per\m\squared}$.
...
\begin{figure}[H]
  ...
  \caption{\DIFaddFL{Predictions for isolated spatial and angular components of discretization error as a function of grid size for both numerical algorithms. Once a target error threshold has been chosen, this figure can be used to estimate the grid sizes that each algorithm would require. The total discretization error is the sum of the spatial and angular parts.}}
  \label{fig:gs_predicted_err}
\end{figure}

\DIFadd{It is important to note that the error analysis in this section has been performed only for a single set of optical properties (HAOCE11).
...
\DIFaddend \section{Optical Conditions for Asymptotics}
...
An in--depth analysis of the error incurred by the numerical asymptotics algorithm is presented in this \DIFdelbegin \DIFdel{Section}\DIFdelend \DIFaddbegin \DIFadd{section}\DIFaddend .
...
Figure \DIFdelbegin \DIFdel{\ref{fig:asym_real_kelp_br01} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:asym_err_vs_b_a01} }\DIFaddend shows this type of comparison for a single value of $a_w$, varying $b$.
...
Therefore, no results with this grid size will show errors below about \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\SI{0.01}{\W\per\m\squared\per\radian}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\SI{0.07}{\W\per\m\squared}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\begin{figure}[H]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\rdfigwidth]{verify_real_kelp_asym_b_scat_ss_sm_th_a05_br01_72x10_rad_err}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\rdfigwidth]{asym_err_vs_b_a01}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Asym real }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Average pointwise difference in irradiance between finite difference and asymptotics solutions for several values of $b$ and $n$ with constant $a=0.1$ in a realistic }\DIFaddendFL kelp \DIFaddbeginFL \DIFaddFL{scenario using a $72\times 10$ grid}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{Blur radius=0.1}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Proper convergence of truncation error is observed between $b=0.2$ and $b=0.5$. Below $b=0.2$, discretization error dominates. Above $b=0.5$, the asymptotic series diverges.}\DIFaddendFL }
  \DIFdelbeginFL %DIFDELCMD < \label{fig:asym_real_kelp_br01}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:asym_err_vs_b_a01}
\DIFaddendFL \end{figure}
...
\begin{figure}[H]
  ...
  \caption{\DIFdelbeginFL \DIFdelFL{asym err vs ab}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Average pointwise difference in irradiance between finite difference and asymptotics solutions for several values of $b$ and $a$ with constant $n=0$ in a realistic kelp scenario using a $72\times 10$ grid. Note that truncation error is smallest for low--scattering, high--absorption cases, and largest for high--scattering, low absorption.}\DIFaddendFL }
...
After a brief transition region, the third order approximation is most accurate, in agreement with Figure \DIFdelbegin \DIFdel{\ref{fig:asym_real_kelp_br01}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:asym_err_vs_b_a01}}\DIFaddend .
...
As mentioned previously, this is because a lower bound on the total error is imposed by the discretization error, as suggested by Figure \DIFdelbegin \DIFdel{\ref{fig:asym_real_kelp_br01}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:asym_err_vs_b_a01}}\DIFaddend .
...
\begin{figure}[H]
  ...
  \caption{\DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL best \DIFdelbeginFL \DIFdelFL{n data vs ab}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{asymptotics solution for each $(a_w,b)$. The value of $n$ used is written in each cell. For high--scattering cases, the $n>0$ terms diverge, so $n=0$ is the best approximation. For most cases, $n=3$ is the best. For very low--scattering cases, discretization error masks the truncation error trend.}\DIFaddendFL }
...
\DIFdelbegin \DIFdel{This concept is explored in Figures \ref{fig:min_n_data_vs_ab_eps01}, \ref{fig:min_n_data_vs_ab_eps005}, and \ref{fig:min_n_data_vs_ab_eps001}.
...
The error of that approximation is shown }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:min_n_data_vs_ab} shows the smallest value of $n$ observed to meet discretization error targets of at most $\bar{\varepsilon}=\SI{1.0}{\W\per\m\squared}$ and $\bar{\varepsilon}=\SI{0.1}{\W\per\m\squared}$ in the left and right columns with the actual average error incurred shown }\DIFaddend on the color axis.
In \DIFdelbegin \DIFdel{each case}\DIFdelend \DIFaddbegin \DIFadd{both cases}\DIFaddend , there are some optical properties for which the error threshold cannot be met for any $n$.
...
\begin{figure}[H]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\rdfigwidth]{min_n_data_vs_ab_eps01}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=6in]{min_n_data_vs_ab}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{min n data vs ab eps01}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{For each $(a_w,b)$, the smallest value of $n$ which satisfies the error criterion shown above is printed in the cell. ``X'' denotes an optical situation in which the criterion cannot be met since adding further terms makes the solution less accurate, not more. As seen on the left the $n=0$ solution suffices for large error criteria.}\DIFaddendFL }
  \DIFdelbeginFL %DIFDELCMD < \label{fig:min_n_data_vs_ab_eps01}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:min_n_data_vs_ab}
\DIFaddendFL \end{figure}

\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\rdfigwidth]{min_n_data_vs_ab_eps005}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{min n data vs ab eps005}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:min_n_data_vs_ab_eps005}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\rdfigwidth]{min_n_data_vs_ab_eps001}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{min n data vs ab eps001}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:min_n_data_vs_ab_eps001}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{\DIFaddbegin \DIFadd{Truncation }\DIFaddend Error Model}
Figure \ref{fig:asym_err_data_xi_model} concisely represents the simulation results for all optical properties and \DIFdelbegin \DIFdel{all }\DIFdelend approximation orders, with error $\varepsilon$ plotted on the vertical axis and $b$ plotted on the color axis.
...
Restated, it seems that \DIFdelbegin \begin{displaymath}
  \DIFdel{\varepsilon \propto \frac{b^{c_1}}{a_w^{c_2}}.
}\end{displaymath}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$\varepsilon \propto \left({b^{c_1}}\right/\left({a_w^{c_2}}\right)$.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Notice how accurately the observed errors are predicted by the model.
This also }\DIFdelend \DIFaddbegin \DIFadd{The accuracy of the prediction }\DIFaddend suggests the definition of a third parameter, as mentioned in Section \ref{sec:iop_study_raw},
\begin{equation}
  \DIFaddbegin \label{eqn:xi}
  \DIFaddend \xi = \frac{b^{3/4}}{a_w^{1/2}},
\end{equation}
...
\begin{figure}[H]
  \centering
  %DIF < \vspace{-1em}
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in]{asym_err_data_xi_model}
%DIFDELCMD <   %%%
%DIF < \vspace{-1em}
  \DIFdelendFL \DIFaddbeginFL \includegraphics[width=5.5in]{asym_err_data_xi_model}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{asym err data }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Truncation error plotted against $a_w$ and }\DIFaddendFL $\xi$ \DIFdelbeginFL \DIFdelFL{model}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for each $n$. Simulations dominated by discretization error have been discarded. The right column shows that $\xi$ sufficiently characterizes an optical situation for the sake of predicting truncation error of the asymptotic approximation. Predicted values, marked with ``x''s, are very accurate.}\DIFaddendFL }
...
For each $n$, the log--log plot displays a clear linear relationship, similar to the pattern seen previously in Figures \DIFdelbegin \DIFdel{\ref{fig:asym_real_kelp_br01} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:asym_err_vs_b_a01} }\DIFaddend and \ref{fig:mms_asym_b_conv}.
...
That point, denoted $(\xi^*, \varepsilon^*)$, \DIFdelbegin \DIFdel{is significant because it }\DIFdelend represents a bifurcation in the convergence behavior of the numerical asymptotics algorithm.
For waters with $\xi < \xi^*$, \DIFdelbegin \DIFdel{adding terms to the series increases the accuracy of the approximation}\DIFdelend \DIFaddbegin \DIFadd{accuracy improves with additional terms}\DIFaddend , whereas for \DIFdelbegin \DIFdel{waters with $\xi$ above the threshold value $\xi^*$, adding terms decreases the }\DIFdelend \DIFaddbegin \DIFadd{with $\xi > \xi^*$, additional terms decrease }\DIFaddend accuracy.
...
\begin{equation}
  \label{eqn:ln_eps_n}
  \ln\DIFaddbegin \left(\DIFaddend \varepsilon_n(\xi)\DIFaddbegin \right) \DIFaddend - \ln\varepsilon^* = (n+1)\left(\ln\xi - \ln\xi^*\right),
\end{equation}
or equivalently,
\begin{equation}
  \label{eqn:eps_n}
...
\begin{align}
  \xi^* &= \DIFdelbegin \DIFdel{1.63}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\SI{1.58}{\m^{-1/4}}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , \\
  \varepsilon^* &= \DIFdelbegin \DIFdel{0.11}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\SI{1.30}{\W\per\m\squared}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ,
\end{align}
\DIFaddbegin \DIFadd{as }\DIFaddend shown in Figure \ref{fig:asym_err_vs_xi_all_n_fit_all}.
...
\begin{figure}[H]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in]{asym_err_vs_xi_all_n_fit_all}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=5.5in]{asym_err_vs_xi_all_n_fit_all}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{asym err vs xi all n fit all}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{All data from the right column of \ref{fig:asym_err_data_xi_model} on a single plot demonstrates a characteristic $\xi=\xi^*$ above which the asymptotic series diverges. This observation permits a simple model for predicting truncation error.}\DIFaddendFL }
...
Figure \DIFdelbegin \DIFdel{\ref{fig:asym_err_model_vs_ab_all_k} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:asym_err_model_vs_ab_all_n} }\DIFaddend shows the predictions of this model for several values of $a_w$, $b$, and $n$, with $\varepsilon$ plotted on the color axis.
...
Also keep in mind that this model and its predictions deal only with truncation error, and that discretization error distorts the actual solution accuracy, as seen in Figures \DIFdelbegin \DIFdel{\ref{fig:asym_real_kelp_br01} }\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:asym_err_vs_b_a01} }\DIFaddend and \ref{fig:best_n_data_vs_ab}.


\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=5in]{asym_err_model_vs_ab_all_k}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{asym err model vs ab all k}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:asym_err_model_vs_ab_all_k}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend This model can now be used to produce general guidelines for the choice of $n$ given a desired error.
...
\DIFaddbegin \begin{figure}[H]
  ...
  \caption{\DIFaddFL{Predicted truncation error for a range of $a_w$ and $b$ values. $n$ varies over plot rows. Linear--linear scale on the left, log--log scale on the right. The asymptotic series converges only for $(a_w,b)$ values to the right of the $\xi^*$ contour.}}
  \label{fig:asym_err_model_vs_ab_all_n}
\end{figure}


\DIFaddend Figure \ref{fig:nbar_model_vs_ab_3eps} shows $\bar{n}$ up to $\bar{n}=3$ for $\bar{\varepsilon}=0.1$, $\bar{\varepsilon}=0.05$, and $\bar{\varepsilon}=0.01$.
...
If memory or computation time requires the numerical asymptotics algorithm to be used, then the $n=0$ approximation should be used, effectively ignoring scattering \DIFaddbegin \DIFadd{in order to avoid a diverging solution}\DIFaddend .
...
\begin{figure}[H]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in]{nbar_model_vs_ab_3eps}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=5.5in]{nbar_model_vs_ab_3eps}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{nbar model vs ab 3eps}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Recommended $n=\bar{n}$ value as a function of $(a_w,b)$ to achieve the truncation error criteria $\bar{\varepsilon}=0.1,0.5,1.0\SI{}{W\per\m\squared}$ for the three plot rows.}\DIFaddendFL }
...
\subsection{\DIFaddbegin \DIFadd{Description of }\DIFaddend Comparison\DIFdelbegin \DIFdel{Results}\DIFdelend }
In \DIFdelbegin \DIFdel{Figures \ref{fig:compare_models_n1}, \ref{fig:compare_models_n2}, and \ref{fig:compare_models_n3}, }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:compare_models_haoce11_both}, }\DIFaddend the light model of this thesis is compared to simpler models under the conditions described in Section \ref{sec:standalone_context} \DIFaddbegin \DIFadd{with the optical properties of coastal California (HAOCE11 from Table \ref{tab:petzold})}\DIFaddend .
In the \DIFdelbegin \DIFdel{figures}\DIFdelend \DIFaddbegin \DIFadd{figure}\DIFaddend , the first model described in this \DIFdelbegin \DIFdel{Chapter}\DIFdelend \DIFaddbegin \DIFadd{chapter}\DIFaddend , which ignores the kelp entirely, is \DIFdelbegin \DIFdel{plotted with a black dashed line.}\DIFdelend \DIFaddbegin \DIFadd{labeled ``No Kelp.''
}\DIFaddend The second model, which uses an additional term in the absorption coefficient to describe the kelp, is \DIFdelbegin \DIFdel{plotted with a dashed blue line.The no--scattering numerical asymptotics solution is plotted in solid red.
Since the above two models do not consider scattering, both the finite difference and asymptotic solutions are plottedfor a range of $b$ values, with the finite difference $b$ described by the left color bar, and the asymptotic $b$ described by the right color bar, so as to distinguish between the two sets of curves}\DIFdelend \DIFaddbegin \DIFadd{labeled ``Simple Kelp.''
...
As described in Section \ref{sec:perceived_irrad}, the perceived irradiance is a weighted average of the irradiance over the kelp distribution, whereas the average irradiance is weighted evenly over the entire horizontal domain}\DIFaddend .

\DIFdelbegin \DIFdel{In each of the three Figures, a different value of $n$ is used for the asymptotic approximations for $b>0$: $n=1$, $n=2$, and $n=3$ for Figures \ref{fig:compare_models_n1}, \ref{fig:compare_models_n2}, and \ref{fig:compare_models_n3} respectively.
In the left column of each figure, }\DIFdelend \DIFaddbegin \DIFadd{Both quantities are plotted with error bands calculated via Richardson extrapolation.
...
Then, the standard 2D Richardson extrapolation procedure was applied independently for each $z$, using the interpolated irradiance value at that $z$ as }\DIFaddend the \DIFdelbegin \DIFdel{average irradiance is plotted as a function of depth for each of the calculations from the light model of this thesis, calculated over the entire horizontal domain.
In the right column, the perceived irradiance, as described in Section \ref{sec:perceived_irrad}, is shown for the asymptotics and finite difference calculations}\DIFdelend \DIFaddbegin \DIFadd{scalar to extrapolate, as described in Section \ref{sec:generalized_re}.
The plotted curves are the irradiances from the $72\times 10$ grid.
The error bands are three times the estimated error in order to make the estimate conservative, as recommended by \mbox{%DIFAUXCMD
\cite{roache_perspective:_1994}}\hspace{0pt}%DIFAUXCMD
.
For a detailed procedure for error reporting in numerical simulations, see \mbox{%DIFAUXCMD
\cite{noauthor_procedure_2008}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

\DIFaddbegin \subsection{\DIFadd{Comparison: Irradiance Curves}}
\DIFaddend First, note that the no--kelp model predicts the highest light levels throughout the domain, as should be expected since it ignores \DIFdelbegin \DIFdel{a significant aspect of the light reduction}\DIFdelend \DIFaddbegin \DIFadd{the dominant source of attenuation}\DIFaddend .
The simple kelp model agrees with the no--kelp model at the surface, but then reduces \DIFdelbegin \DIFdel{more }\DIFdelend quickly once the kelp, which \DIFdelbegin \DIFdel{is largest }\DIFdelend \DIFaddbegin \DIFadd{has a maximum frond length }\DIFaddend at $z=2$, \DIFdelbegin \DIFdel{is introduced}\DIFdelend \DIFaddbegin \DIFadd{becomes sufficiently dense}\DIFaddend .
At the bottom of the domain where there is little kelp remaining, the two curves are seen to be parallel in the log--linear scale, showing that the absorption coefficients are \DIFdelbegin \DIFdel{equal }\DIFdelend \DIFaddbegin \DIFadd{roughly equal, }\DIFaddend as at the surface.
\DIFdelbegin \DIFdel{Looking at the left column, the average irradiance from no--scattering solution to the }\DIFdelend \DIFaddbegin \DIFadd{The average irradiance }\DIFaddend 3D model appears to agree quite well with the simple kelp model, especially near the surface and bottom, where both models are approximately reduced to the first model.
In the $z=2$ region where the most kelp is present, \DIFaddbegin \DIFadd{though, }\DIFaddend the 3D model predicts higher absorption than the simpler 1D model.
However, between $z=3$ and $z=4$, the \DIFaddbegin \DIFadd{light in the }\DIFaddend 3D kelp model \DIFdelbegin \DIFdel{decreases }\DIFdelend \DIFaddbegin \DIFadd{attenuates }\DIFaddend more slowly than the 1D model, perhaps because light is able to penetrate the water in regions where the kelp is not present, and the multitude of angles in which the light travels allow some of the rays to reach even the lower depths without being absorbed by the kelp.

\DIFdelbegin \DIFdel{As for the right column, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend perceived irradiance is clearly much lower than the average irradiance and the other models.
This makes sense, as the light is dimmest in the regions of highest kelp density, and the perceived irradiance weights these regions the highest, practically ignoring the edges of the domain where light is absorbed only by the water; these regions skew the average irradiance upward\DIFdelbegin \DIFdel{although }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend they are not actually representative of the light field where photosynthesis is occurring.
From this point of view, it seems that the other two light models significantly overestimate the amount of light available where it actually \DIFdelbegin \DIFdel{matters}\DIFdelend \DIFaddbegin \DIFadd{of interest}\DIFaddend .

\DIFdelbegin \DIFdel{Considering the finite difference solution, the most visible effect of increasing scattering is to decrease the light field towards the bottom of the domain.
Of course, scattering does not cause that light to be lost entirely.
Rather, it tends to reflect it towards the surface, where increases in scattering lead to higher light levels}\DIFdelend \DIFaddbegin \DIFadd{Note that the 3D model does not predict the lowest light fields everywhere; the average irradiance is observed to be larger than the no--kelp irradiance in the second plot in Figure \ref{fig:compare_models_haoce11_both}}\DIFaddend .
This is \DIFdelbegin \DIFdel{noteworthy since the light available very close }\DIFdelend \DIFaddbegin \DIFadd{due to the effect of scattering shifting light from lower in the domain closer }\DIFaddend to the surface\DIFdelbegin \DIFdel{is significantly higher than it is a few meters below}\DIFdelend .
This tendency for scattering to insulate the upper region of the water may prove to be important in understanding how photosynthesis near the ocean's surface differs from photosynthesis at \DIFdelbegin \DIFdel{greater depths.
}\DIFdelend \DIFaddbegin \DIFadd{depth.
}\DIFaddend 

\DIFaddbegin \subsection{\DIFadd{Comparison: Total Radiant Flux}}
\DIFadd{Figure \ref{fig:compare_models_haoce11_flux} shows the total radiant flux in Watts predicted by each model, calculated by integrating each depth--dependent light field over the vertical domain, }\textit{\DIFadd{weighted by the frond area density}} \DIFadd{(frond area per vertical meter).
...
\DIFadd{The radiant flux predicted by the 3D average irradiance is just 13\% of the flux disregarding kelp, and 57\% of the simple kelp flux.
...
\DIFaddend \begin{figure}[H]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in]{compare_models_n1}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=5.75in]{compare_models_haoce11_both}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Compare }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Average irradiance and perceived irradiance from finite difference compared to simpler light }\DIFaddendFL models \DIFdelbeginFL \DIFdelFL{n=1}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for the case of coastal California water (HAOCE11) with realistic kelp growth. Frond lengths over depth are shown on the right--hand axis. Discretization error is shown by error bands for perceived irradiance and average irradiance. Note the different scales in the two plots.}\DIFaddendFL }
  \DIFdelbeginFL %DIFDELCMD < \label{fig:compare_models_n1}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:compare_models_haoce11_both}
\DIFaddendFL \end{figure}
\DIFaddbegin 

\DIFaddend \begin{figure}[H]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in]{compare_models_n2}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=6in]{compare_models_haoce11_flux}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Compare models n=2}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Total radiant flux in Watts through kelp predicted by each model. The magnitude of the differences highlight the importance of considering the three--dimensional spatial kelp distribution.}\DIFaddendFL }
  \DIFdelbeginFL %DIFDELCMD < \label{fig:compare_models_n2}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:compare_models_haoce11_flux}
\DIFaddendFL \end{figure}
\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=6in]{compare_models_n3}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Compare models n=3}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:compare_models_n3}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend 
...
\begin{itemize}
  ...
  \item Fronds \DIFdelbegin \DIFdel{eminate }\DIFdelend \DIFaddbegin \DIFadd{emanate }\DIFaddend from an infinitely thin, perfectly vertical rope, with no stipe.
  \item Population frond lengths are normally distributed in each depth layer.
  \item Fronds are oriented according to a von Mises distribution whose sharpness is proportional to current velocity and \DIFdelbegin \DIFdel{independend }\DIFdelend \DIFaddbegin \DIFadd{independent }\DIFaddend of frond length.
...
Rather than integrating the two dimensional \DIFdelbegin \DIFdel{length-orientation }\DIFdelend \DIFaddbegin \DIFadd{length--orientation }\DIFaddend distribution from
...
\DIFdelbegin \DIFdel{Finally, }\DIFdelend \DIFaddbegin \DIFadd{Also, it is important to note that in reality, }\DIFaddend photosynthesis is a frequency--dependent process, and a frequency--dependent light model may therefore significantly improve seasonal growth predictions.
...
\DIFaddbegin \DIFadd{Finally, a major outstanding task is the experimental validation of the model presented in this thesis.
Chapter }\Rom{\ref{chap:model_analysis}} \DIFadd{deals with ensuring that the numerical algorithms presented here }\textit{\DIFadd{solve the equations right}}\DIFadd{, but it remains to be shown whether they }\textit{\DIFadd{solve the right equations}}\DIFadd{.
...
\DIFaddend \subsection{Numerical Improvements}
...
MPI is a protocol which facilitates \DIFdelbegin \DIFdel{work-sharing }\DIFdelend \DIFaddbegin \DIFadd{work--sharing }\DIFaddend between processes either on the same machine or over a network.
...
However, network communication is often significantly slower than \DIFdelbegin \DIFdel{inter-thread }\DIFdelend \DIFaddbegin \DIFadd{inter--thread }\DIFaddend communication.
...
Among the possible options are the Lebedev grid and ``spherical t--design''\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{beentjes_quadrature_nodate,an_numerical_2016,beckmann_local_2014}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{an_numerical_2016,beckmann_local_2014,beentjes_quadrature_nodate}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\subsection{\DIFaddbegin \DIFadd{Application to }\DIFaddend Seaweed Cultivation\DIFdelbegin \DIFdel{Simulations}\DIFdelend }
...
Therefore, \DIFdelbegin \DIFdel{modelling }\DIFdelend \DIFaddbegin \DIFadd{modeling }\DIFaddend the optical properties of such water as a function of nutrient concentrations in order to run appropriate kelp growth simulations may prove valuable for addressing such concerns.
...
\DIFaddbegin \bibitem{noauthor_procedure_2008}
\DIFadd{Procedure for }{\DIFadd{Estimation}} \DIFadd{and }{\DIFadd{Reporting}} \DIFadd{of }{\DIFadd{Uncertainty}} {\DIFadd{Due}} \DIFadd{to
  }{\DIFadd{Discretization}} \DIFadd{in }{\DIFadd{CFD}} {\DIFadd{Applications}}\DIFadd{.
}\newblock {\em \DIFadd{Journal of Fluids Engineering}}\DIFadd{, 130(7):078001--078001--4, July
  2008.
}

\bibitem{an_numerical_2016}
...
\DIFaddbegin \bibitem{foldal_morphological_2018}
\DIFadd{S.~Foldal.
}\newblock {\em \DIFadd{Morphological relations of cultivated }{\DIFadd{Saccharina}} \DIFadd{latissima at
  three stations along the }{\DIFadd{Norwegian}} \DIFadd{coast}}\DIFadd{.
}\newblock \DIFadd{Master's thesis in }{\DIFadd{Marine}} {\DIFadd{Coastal}} {\DIFadd{Development}}\DIFadd{, Norwegian
  University of Science and Technology, Trondheim, Norway, 2018.
}

\DIFaddend \bibitem{hadley_modeling_2015}
...
\DIFaddbegin \bibitem{kirk_light_1994}
\DIFadd{J.~T.~O. Kirk.
}\newblock {\em \DIFadd{Light and }{\DIFadd{Photosynthesis}} \DIFadd{in }{\DIFadd{Aquatic}} {\DIFadd{Ecosystems}}}\DIFadd{.
}\newblock \DIFadd{Cambridge University Press, Apr. 1994.
}\newblock \DIFadd{Google-Books-ID: It5GePwa2EIC.
}

\bibitem{message_passing_interface_forum_mpi:_1993}
...
\bibitem{meurer_sympy:_2017}
\DIFadd{A.~Meurer, C.~P. Smith, M.~Paprocki, S.~B. Kirpichev, and M.~Rocklin.
}\newblock {\DIFadd{SymPy}}\DIFadd{: symbolic computing in }{\DIFadd{Python}}\DIFadd{.
}\newblock \DIFadd{page~27, 2017.
}
...
\DIFaddbegin \bibitem{roache_perspective:_1994}
\DIFadd{P.~J. Roache.
}\newblock \DIFadd{Perspective: }{\DIFadd{A}} {\DIFadd{Method}} \DIFadd{for }{\DIFadd{Uniform}} {\DIFadd{Reporting}} \DIFadd{of }{\DIFadd{Grid}}
  {\DIFadd{Refinement}} {\DIFadd{Studies}}\DIFadd{.
}\newblock {\em \DIFadd{Journal of Fluids Engineering}}\DIFadd{, 116(3):405--413, Sept. 1994.
}

\bibitem{roache_verification_1998-1}
\DIFadd{P.~J. Roache.
}\newblock {\em \DIFadd{Verification and validation in computational science and
  engineering}}\DIFadd{.
}\newblock \DIFadd{Hermosa, Albuquerque, NM, 1998.
}

\bibitem{roache_verification_1998}
\DIFadd{P.~J. Roache.
}\newblock \DIFadd{Verification of }{\DIFadd{Codes}} \DIFadd{and }{\DIFadd{Calculations}}\DIFadd{.
}\newblock {\em \DIFadd{AIAA Journal}}\DIFadd{, 36(5):696--702, May 1998.
}

\bibitem{roache_building_2004}
\DIFadd{P.~J. Roache.
}\newblock \DIFadd{Building }{\DIFadd{PDE}} \DIFadd{codes to be verifiable and validatable.
}\newblock {\em \DIFadd{Computing in Science Engineering}}\DIFadd{, 6(5):30--38, Sept. 2004.
}

\bibitem{roache_editorial_1986}
\DIFadd{P.~J. Roache, K.~N. Ghia, and F.~M. White.
}\newblock \DIFadd{Editorial }{\DIFadd{Policy}} {\DIFadd{Statement}} \DIFadd{on the }{\DIFadd{Control}} \DIFadd{of }{\DIFadd{Numerical}}
  {\DIFadd{Accuracy}}\DIFadd{.
}\newblock {\em \DIFadd{Journal of Fluids Engineering}}\DIFadd{, 108(1):2--2, Mar. 1986.
}

\DIFaddend \bibitem{saad_gmres:_1985}
...
%DIFDELCMD < %%%
\DIFdelend For the polar angle, we similarly let
\begin{equation*}
  \phi_m = (m-1)d\phi.
\end{equation*}
\DIFdelbegin %DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend Since the polar azimuthal is not periodic, we also store the endpoint, so
\begin{align*}
  \phi_1 &= 0, \\
  \phi_{n_\phi} &= \pi.
\end{align*}
...
%DIFDELCMD < %%%
\DIFdelend It is also useful to define the edges between angular grid cells as
\begin{alignat}{3}
...
\DIFaddend Note that while $\theta$ has its final edge following its final center, this is
not the case for $\phi$, as seen in Figure \ref{fig:angular_grid_plots}.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=.75\linewidth]{angular_grid_plots}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Angular grid}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:angular_grid_plots}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Because angles are indexed by a single integer $p$, there is a \DIFdelbegin \DIFdel{one-to-one }\DIFdelend \DIFaddbegin \DIFadd{one--to--one }\DIFaddend relationship between
...
is first necessary to construct a \DIFdelbegin \DIFdel{one-dimensional }\DIFdelend \DIFaddbegin \DIFadd{one--dimensional }\DIFaddend piecewise constant integrand
...
To find the location of discontinuities in the \DIFdelbegin \DIFdel{itegrand}\DIFdelend \DIFaddbegin \DIFadd{integrand}\DIFaddend , we first calculate the
...
\begin{align}
  \sigma_x = (\delta_x+1)/2\DIFaddbegin \DIFadd{, }\DIFaddend \\
  \sigma_y = (\delta_y+1)/2\DIFaddbegin \DIFadd{, }\DIFaddend \\
  \sigma_z = (\delta_z+1)/2\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align}
\DIFaddbegin \DIFadd{simply to keep track of the ray direction.
}\DIFaddend 

For this algorithm, we keep two sets of indices. $(i,j,k)$ indexes the grid
cell, and \DIFdelbegin \DIFdel{will be }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend used for extracting physical quantities from each cell along
the path.
Meanwhile, $(i^e,j^e,k^e)$ \DIFdelbegin \DIFdel{will index }\DIFdelend \DIFaddbegin \DIFadd{indexes }\DIFaddend the edges between grid cells, beginning
after the first cell. \DIFdelbegin \DIFdel{i.e.}\DIFdelend \DIFaddbegin \DIFadd{That is}\DIFaddend , $i^e=1$ refers not to the plane $x=\xmin$, but to $x=\xmin+dx$.
...
required to cross the next $x$, $y$, and $z$ \DIFdelbegin \DIFdel{edge-planes}\DIFdelend \DIFaddbegin \DIFadd{edge--planes}\DIFaddend .
...
Using the python package Sympy \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{meurer_sympy:_2017}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , the boundary conditions and source function are calculated to be
\begin{align}
...
\DIFaddbegin \savebox{\memtablebox}{\begin{tabular}{@{}r@{}l@{\!}}
&\DIFadd{\,\,$n_a$}\\[-1em] \DIFadd{$n_s$
}\end{tabular}}
\DIFaddend 

The \DIFdelbegin \DIFdel{matrix size in memory in bytes is calculated as follows.
}%DIFDELCMD < \begin{lstlisting}%DIFDELCMD < [language=Python]
%DIFDELCMD < def calc_size(ns, na):
%DIFDELCMD <     nx = ns
%DIFDELCMD <     ny = ns
%DIFDELCMD <     nz = ns
%DIFDELCMD <     ntheta = na
%DIFDELCMD <     nphi = na
%DIFDELCMD <     nomega = ntheta * (nphi-2) + 2
%DIFDELCMD < 

%DIFDELCMD <     nnz = nx * ny * nomega * ( nz * (6 + nomega) - 1)
%DIFDELCMD < 

%DIFDELCMD <     return 8*nnz
%DIFDELCMD < \end{lstlisting}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{number of elements in the finite difference coefficient matrix is given by Equation }\eqref{eqn:nnz}\DIFadd{.
Assuming that double precision floating point numbers are used, the number of bytes required to store the matrix values is found by multiplying the result of }\eqref{eqn:nnz} \DIFadd{by 8 bytes per number.
These values are listed in Table \ref{tab:mem_store}.
}\DIFaddend 

\DIFdelbegin \savebox{\memtablebox}{%DIFDELCMD < \begin{tabular}{@{}r@{}l@{\!}}
%DIFDELCMD < &%%%
\DIFdel{\,\,$n_a$}%DIFDELCMD < \\[-1em] %%%
\DIFdel{$n_s$
...
%DIFDELCMD < %%%
\DIFdel{Memory }\DIFdelend \DIFaddbegin \DIFadd{Since the matrix is sparse, the indices of the elements must also be stored.
Due to the large number of elements involved, long integers must be used as indices, which also require 8 bytes to store.
The exact amount of memory required depends on the storage scheme, which in this case is the Compressed Sparse Row (CSR) format provided by LIS \mbox{%DIFAUXCMD
...
From personal experience, the memory }\DIFaddend requirement for LIS solution via GMRES\DIFdelbegin \DIFdel{with restart=}\DIFdelend \DIFaddbegin \DIFadd{, restarted every }\DIFaddend 100 \DIFdelbegin \DIFdel{is well-estimated }\DIFdelend \DIFaddbegin \DIFadd{iterations, is well--estimated }\DIFaddend by multiplying the \DIFdelbegin \DIFdel{above }\DIFdelend \DIFaddbegin \DIFadd{values in Table \ref{tab:mem_store} }\DIFaddend by 5.
\DIFaddbegin \DIFadd{These solution memory estimates are listed in Table \ref{tab:mem_solve}.
}\DIFaddend 
...
The full \DIFdelbegin \DIFdel{FORTRAN }\DIFdelend \DIFaddbegin \DIFadd{Fortran }\DIFaddend implementation of the model described in this thesis.
\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{The up--to--date version of this }\DIFaddend code can be found online at
\DIFdelbegin \DIFdel{the following URLs:
}%DIFDELCMD < 

%DIFDELCMD < \begin{verbatim}%DIFDELCMD < 
%DIFDELCMD < https://github.com/OliverEvans96/kelp
%DIFDELCMD < https://gitlab.com/OliverEvans96/kelp
%DIFDELCMD < \end{verbatim}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \texttt{\DIFadd{https://github.com/OliverEvans96/kelp}}\DIFadd{.
}\DIFaddend 
