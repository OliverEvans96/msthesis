...
Assuming a linear relationship between the current velocity and the steepness of the angular distribution, define the \textit{alignment coefficient} $\eta$, with units of inverse velocity(\SI{}{\s\per\m}).
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.6\linewidth]{kelp_sample}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.6\linewidth]{shade_area}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.6\linewidth]{cart_shade}
...
Multiplying $\tilde{P}_k$ by the number of fronds in the population $n$ of the depth layer gives the expected number of fronds occupying the point.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.6\linewidth]{prob_shade}
	\captionof{figure}{Colored plot of the probability of frond occupation sampled at 121 points using $\theta_f=2\pi/3,\eta v_w=1$}
